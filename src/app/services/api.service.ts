...
export class ApiService {
  ...
  private readonly _searchParam = new BehaviorSubject<string>('');

  /**
   * @value is UPPERCASE
   */
  public readonly searchParam$ = this._searchParam
    .asObservable()
    .pipe(map((value) => value.toUpperCase()));

  notifySearchParam(value: string) {
    this._searchParam.next(value);
  }
  ...
  getPokemons(): Observable<List> {
    ...
    return this.httpClient.get<List>(
      `${environment.APY_URL}/pokemon?limit=100000&offset=0`
    );
  }
}
